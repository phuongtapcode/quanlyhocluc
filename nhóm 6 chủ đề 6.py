...
def in_danh_sach(ds):
    ...
    print("-" * 100)
    for i in ds_sapxep:
        dong = ten_dai(i['ten'], 30)
        print(f"{i['maso']:<10} {dong[0]:<30} {i['dcs']:<15.1f} {i['dcn']:<20.1f} {i['dck']:<15.1f}")
        for du in dong[1:]:
            print(f"{'':<10} {du:<30}")
    print()
    input("\nNhấn Enter để quay về menu...\n")
...
def diem_cao_nhat(ds):
    ...
    print("-" * 100)
    for sv in sv_cao:
        dong = ten_dai(sv['ten'], 30)
        print(f"{sv['maso']:<10} {dong[0]:<30} {sv['dck']:.1f}")
        for du in dong[1:]:
            print(f"{'':<10} {du:<30}")
    print()
    input("\nNhấn Enter để quay về menu...\n")
...
def khong_dat(ds):
    ...
    for i in ds:
        ...
        if cacmon:
            dong = ten_dai(i['ten'], 30)
            print(f"{i['maso']:<10} {dong[0]:<30} {cacmon[0][0]:<20} {cacmon[0][1]:.1f}")
            for du in dong[1:]:
                print(f"{'':<10} {du:<30}")
...
def nhap_danh_sach_them(ds):
    while True:
        ...
        if maso.lower() == "thoat":
            ...
            print("Mã số không vượt quá 10 kí tự")
...
def xoa_thong_tin(ds):
    ...
    while True:
        ...
        dong = ten_dai(thongtinsv['ten'], 30)
        print(f"{thongtinsv['maso']:<10} {dong[0]:<30}  {thongtinsv['dcs']:<15.1f}  {thongtinsv['dcn']:<20.1f}  {thongtinsv['dck']:<15.1f}")
        for du in dong[1:]:
            print(f"{'':<10}  {du:<30}")
        while True:
            print("\n1. Xóa \n2. Hủy")
...
danhsach = [{"maso": "MS0001", "ten": "Nguyen Van A", "dcs": 10.0, "dcn": 10.0, "dck": 10.0 * 0.4 + 10.0 * 0.6},
            {"maso": "MS0002", "ten": "Ngueyn Van B", "dcs": 1.0, "dcn": 10.0, "dck": 1.0 * 0.4 + 10.0 * 0.6},
            {"maso": "MS0003", "ten": "Ngueyn Van C", "dcs": 1.0, "dcn": 1.0, "dck": 1.0 * 0.4 + 1.0 * 0.6},
            {"maso": "MS0004", "ten": "Ngueyn Van D", "dcs": 9.0, "dcn": 10.0, "dck": 9.0 * 0.4 + 10.0 * 0.6}]
